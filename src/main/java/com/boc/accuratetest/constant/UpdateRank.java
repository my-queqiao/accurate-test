...
@Component
@Order(value = 1)
public class UpdateRank implements CommandLineRunner {
	@Autowired
	private PermissionBiz permissionBiz;
	@Override
	public void run(String... args) throws Exception {
		String packageName = "com.boc.accuratetest.acl";
		List<Class<?>> list = getClasses(packageName);
		List<Permission> ps = new ArrayList<>();
		for (Class<?> c : list) {
			Permission p = new Permission();
			p.setRankName(c.getName());// 类全名
	...
	public static void main(String[] args) throws IOException {
		...
		for (Class<?> c : list) {
			...
			String value = c.getAnnotation(SecurityAclDesc.class).value();
